...
type diff = Equal of string | Added of string | Deleted of string
type word = diff * string

type assembled_line =
  | CommonWord of string
  | AddedWord of word list
  | DeletedWord of word list

type hunk = assembled_line list
type origin = Mine | Their
type line_content = Change of string | Common of string | Empty
type line = origin * line_content

let line_to_string (line : line) : string =
  match line with _, Change s -> s | _, Common s -> s | _, Empty -> ""

let split_into_words (line : string) : string array =
  let words = String.split_on_char ' ' line in
  Array.of_list words

let split_hunk (hunk : Patch.hunk) : line list * line list =
  let rec process_lines (mine_acc : line list) (their_acc : line list) =
    function
    | [] -> (List.rev mine_acc, List.rev their_acc)
    | `Common line :: rest ->
        process_lines
          ((Mine, Common line) :: mine_acc)
          ((Their, Common line) :: their_acc)
          rest
    | `Mine line :: rest ->
        process_lines
          ((Mine, Change line) :: mine_acc)
          ((Their, Empty) :: their_acc)
          rest
    | `Their line :: rest ->
        process_lines
          ((Mine, Empty) :: mine_acc)
          ((Their, Change line) :: their_acc)
          rest
  in
  process_lines [] [] hunk.Patch.lines

(* Diffing for Line Type *)
module LineDiff = struct
  type t = line

  let compare line1 line2 =
    let s1 = line_to_string line1 in
    let s2 = line_to_string line2 in
    String.compare s1 s2
end

module Simple_diff_line = Simple_diff.Make (LineDiff)

let line_diff (lines1 : line list) (lines2 : line list) :
    Simple_diff_line.diff list =
  let arr1 = Array.of_list lines1 in
  let arr2 = Array.of_list lines2 in
  Simple_diff_line.get_diff arr1 arr2
  ...
  let rec process_hunk (mine_acc : line list) (their_acc : line list) = function
    ...
    | changes ->
        let rec buffer_changes (mine : line list) (their : line list) = function
          | `Mine line :: rest ->
              buffer_changes ((Mine, Change line) :: mine) their rest
          | `Their line :: rest ->
              buffer_changes mine ((Their, Change line) :: their) rest
        ...
        let pad_and_append (orig_acc : line list) (changes : line list) :
          ...
          let rec pad_append (acc : line list) (i : int) : line list =
            if i < max_len then
              ...
              else pad_append ((Mine, Empty) :: acc) (i + 1)
            else acc
          in
